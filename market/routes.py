    ...
    if request.method == "POST":
        ...
        if p_item_object:
            if current_user.can_purchase(p_item_object):
                ...
                flash(
                    f"Unfortunately, you don't have enough credits to purchase this {p_item_object.name}",
                    category="danger",
                )
        # Sell item logic
        sold_item = request.form.get(
            "sold_item"
        )  # sold_item from owned_items_modals file name
        s_item_object = Item.query.filter_by(name=sold_item).first()
        if s_item_object:
            if current_user.can_sell(s_item_object):
                s_item_object.sell(current_user)
                flash(
                    f"Congratulations! You have sold the {s_item_object.name} back to market .",
                    category="success",
                )
            else:
                flash(
